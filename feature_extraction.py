...
class Sentence(object):
    ...
    def get_legal_labels(self, dep_offset=1):
        labels = ([1] if len(self.stack) > 2 else [0])*dep_offset
        labels += ([1] if len(self.stack) >= 2 else [0])*dep_offset
    ...
    def get_transition_from_current_state(self, dep2idx=None, dep_offset=1):  # logic to get next transition
        ...
        if stack_token_1.token_id >= 0 and stack_token_1.head_id == stack_token_0.token_id:  # left arc
            ...
            return 1 if not dep2idx else dep_offset+dep2idx[stack_token_0.dep] # right arc
        else:
...
class Dataset(object):
    ...
    def convert_data_to_ids(self):
        self.train_inputs, self.train_targets = self.feature_extractor. \
            create_instances_for_data(self.train_data, self.word2idx, self.dep2idx)
...
class FeatureExtractor(object):
    ...
    def extract_for_current_state(self, sentence, word2idx, dep2idx):
        ...
        return [word_input_ids, dep_input_ids]  # 48 features


    def create_instances_for_data(self, data, word2idx, dep2idx):
        lables = []
        word_inputs = []
        num_label_classes = (self.model_config.num_classes-1)*len(dep2idx)+1 if \
                    self.model_config.num_classes == 3 else \
                    self.model_config.num_classes
        for i, sentence in enumerate(data):
            num_words = len(sentence.tokens)

            for _ in range(num_words * 2):
                word_input, dep_label = self.extract_for_current_state(sentence, word2idx, dep2idx)
        ...
        targets = np.zeros((len(lables), num_label_classes), dtype=np.int32)
        targets[np.arange(len(targets)), lables] = 1

        return [word_inputs], targets
...
def load_datasets(load_existing_dump=False):
    ...
    if load_existing_dump:
        ...
        dataset.model_config.num_classes = (dataset.model_config.num_classes-1) * \
                                            len(dataset.dep2idx) + 1
