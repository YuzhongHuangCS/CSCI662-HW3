...
from utils.general_utils import get_vocab_dict
import argparse
import pdb
import pickle
...
class Token(object):
    def __init__(self, token_id, word, pos, dep, head_id, line=None):
    ...
    def create_instances_for_data(self, data, word2idx, pos2idx, dep2idx, idx2dep):
        ...
        for i, sentence in enumerate(data):
            ...
            for _ in range(num_words * 2):
                ...
                labels.append(get_dep_from_idx(curr_transition))
        ...
        #targets = np.zeros((len(labels), num_label_classes), dtype=np.int32)
...
def load_datasets(filename):
    ...
    dataset = Dataset(model_config, train_data, None, None, feature_extractor)

    dataset.build_vocab()
    dataset.model_config.num_classes = (dataset.model_config.num_classes-1) * len(dataset.dep2idx) + 1
